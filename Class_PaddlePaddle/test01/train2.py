...
#定义数据
datatype="float32"
train_data=numpy.array([[1],[2],[3],[4],[5],[10]]).astype(datatype)#10倍缩放 此处数据类型尽可能与网格类型相似
y_true = numpy.array([[13],[-23],[33],[-43],[53],[-103]]).astype(datatype)
#定义网络
x = fluid.layers.data(name="x",shape=[1],dtype=datatype)
y = fluid.layers.data(name="y",shape=[1],dtype=datatype)
z = fluid.layers.fc(input=x,size=1,act="tanh")
y_predict = fluid.layers.fc(input=z,size=1,act=None)#定义x与其有关系
#定义损失函数
